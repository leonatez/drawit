...
export default function AuthModal() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // If redirect was blocked by the browser, re-enable the button
      setLoading(false);
    } catch (err: unknown) {
        ...
        {mode !== 'forgot' && !forgotSent && (
          <>
            <button
              ...
              disabled={loading}
              className="w-full flex items-center justify-center gap-2 bg-white text-[#1e293b] rounded-lg py-2.5 text-sm font-medium disabled:opacity-50 hover:bg-gray-100 transition-colors"
            >
